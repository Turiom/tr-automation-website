...
(function () {
  ...
  function size() {
    // Ohne 2D-Kontext nichts tun: getContext darf null liefern, und ein spaeteres Resize warf
    // dann bei ctx.setTransform (Review 12.09.2026, Test tests/motion.test.cjs).
    if (!cv || !ctx) return;
  ...
  function draw(animate) {
    ...
    for (var i = 0; i < dots.length; i++) {
      ...
      // Bei reduzierter Bewegung steht das Feld still, auch beim Scrollen (keine Parallaxe).
      var par = reduce ? 0 : 0.04 + p.d * 0.22;  // nahe Dreiecke laufen beim Scrollen schneller mit
    ...
    for (var b = 0; b < bigs.length; b++) {
      ...
      var by = (((o.y * H - sy * (reduce ? 0 : 0.3)) % H) + H) % H;
  ...
  // Jede gestartete Schleife traegt eine Laufnummer. stop() erhoeht sie, damit ein bereits
  // vorgemerkter Frame der alten Schleife beim Ausfuehren verfaellt. Vorher liefen nach schnellem
  // Verbergen und Wiederzeigen der Seite zwei Schleifen parallel (Review 12.09.2026).
  var lauf = 0;
  function loop(nummer) {
    return function tick() {
      if (!running || nummer !== lauf) return;
      draw(true);
      raf(tick);
    };
  }
  ...
  function stop() { running = false; lauf++; }
